import numpy as np
import pandas as pd
import matplotlib.pyplot as plt # หาเรื่องใช้ด้วย plot ออกมาดู
...
# โหลดข้อมูล
path = "Air_quality/updated_pollution_dataset.csv"
...
model = Sequential([
    ...
    Dense(len(np.unique(y)), activation='softmax')  # Output Layer ใช้ Softmax สำหรับ Multi-class Classification
])
...
# Train Model
# ทดสอบ epochs 30, 50, 70, 100 และ batch_size 16, 32
...
# shap.summary_plot(shap_values, features=X_test, feature_names=data.drop(columns=['Air Quality']).columns, max_display=len(data.drop(columns=['Air Quality']).columns))
